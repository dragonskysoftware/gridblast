/*
 * Game.cpp
 * Implements a static class that runs the Gridblast game
...
bool Game::start() {
	...
	//load the SDL_mixer formats
	int fflags = MIX_INIT_OGG; //the format flags
	int result = Mix_Init(fflags); //init the formats specified by the flags and return the result
	if((result & fflags) != fflags) { //if the init failed
...
void Game::run() {
	message = new TextComponent(SCREEN_WIDTH / 7, SCREEN_HEIGHT / 2, std::string(DEV_MSG), "../assets/fonts/BrokenGlass.ttf", 36, 0, 0, 0, renderer); 
...
void Game::displaySplash() {
	...
	SoundEffect* intro = new SoundEffect("../assets/sounds/Intro.ogg"); //create a new sound effect object containing the intro
	intro->play(); //play the music
